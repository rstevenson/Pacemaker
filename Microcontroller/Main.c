...
unsigned long Tm_sVRP=0;

unsigned long Tm_pVRP=0;

unsigned long Tm_vPace=0;
...
void main(void) {
	...
	//Timer 3 = Pacing
     ...
     OSCCONbits.IDLEN = 1;
     Sleep(); //makes the microcontroller sleep after data is processed, waits for more data or timer interrupt
    }
}
